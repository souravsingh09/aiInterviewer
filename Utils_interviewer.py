...
llm = ChatOpenAI(
    ...
    openai_api_base = "http://10.179.82.226:8030/v1",
...
{topics_wise_interaction_vital[topic]}
        ...
        if len(scores.values()) == 0:
            ...
            vital_eval = sum(scores.values())/len(self.info["vital_topics"])
...
{topics_wise_interaction_good[topic]}
        ...
        if len(scores.values()) == 0:
            ...
            good_eval = sum(scores.values())/len(self.info["good_to_know_topics"])

        final_eval = (2*vital_eval + good_eval)/3

        combined_eval += f"\nThe final evaluation score is {final_eval*100:.2f}%"

        beautify_prompt = f"""
Reformat the evaluation provided to make it more readable and presentable. For each score give a brief explanation of the score and the reason for the score.
EVALUATION:
{combined_eval}
Return the improved evaluation Report within triple backticks. Include the final score as mentioned in percentage.
"""
        result = llm.invoke(beautify_prompt).content
        print(combined_eval)
